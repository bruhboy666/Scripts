...
if not CarCollection or not rF or not rE then return err("Wrong game") else Notification.Notify("Car Crushers 2", "made by Bruhboy666", Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)); wait(1) end -- check
...
local window = library:CreateWindow("Car Crushers 2", Vector2.new(450,330), Enum.KeyCode.RightShift)
    ...
    local miscTab = window:CreateTab("Misc")
        ...
        local discSector = miscTab:CreateSector("Discord", 'left')
            ...
            copydisc_button = discSector:AddButton("Copy to Clipboard", function() if setclipboard then setclipboard('https://discord.gg/rgg6RdtSUJ') else print("DnyxZRwQh3") end end)
                
        local changeSector = miscTab:CreateSector("Changelogs", 'left')
            changeSector:AddLabel("• New Script !")
            changeSector:AddLabel("")         
            changeSector:AddLabel("")